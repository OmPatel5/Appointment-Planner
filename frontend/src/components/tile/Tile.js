    ...
    <div className={props.isAppointment ? "tile-container-appointment" : "tile-container"}>
      {
        ...
        :
        <>
        <p className="tile tile-title">{secondElement} <img className='editButton' onClick={(e) => {setIsEdit(true); props.setIsEditContact(info)}} src={require('./editIcontest.png').default} alt='' /><button className="remove" onClick={() => props.isAppointment ? props.removeAppointment(info) : props.removeContact(info)}>x</button></p>
        <hr/>
        {values.map((val, index) => (
