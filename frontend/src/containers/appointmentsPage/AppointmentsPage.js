  ...
  return (
    <div>
      ...
      <section style={{marginTop: '100px', background: 'rgba(0, 0, 0, 0.5)', borderRadius: "5px", padding: " 50px 50px 70px"}}>
        ...
        <AppointmentForm title={title} contact={contact} contacts={props.contacts} date={date} time={time} 
      ...
      <section style={{background: 'rgba(0, 0, 0, 0.5)', padding: " 50px 50px 70px", borderRadius: "5px"}}>
        <h2>Appointments</h2>
        <TileList isAppointment={true} renderInformation={props.appointments} isEdit={props.isEdit} 
        onSubmit={props.onSubmit} setIsEdit={props.setIsEdit} removeAppointment={props.removeAppointment} 
        isEditContact={props.isEditContact} setIsEditContact={props.setIsEditContact}
        contacts={props.contacts} handleSubmit={handleSubmit} />
      </section>
    </div>
  );
};
